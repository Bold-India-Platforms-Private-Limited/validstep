...
function NavItem({ to, icon: Icon, label, onClick }) {
  return (
    <NavLink
      ...
      onClick={onClick}
      className={({ isActive }) =>
        `flex items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium transition-colors ${isActive ? 'bg-violet-50 text-violet-700' : 'text-slate-600 hover:bg-slate-100'}`
      }
    >
  ...
  const Sidebar = () => (
    <div className="flex h-full flex-col">
      ...
      <div className="border-t border-slate-100 p-3">
        ...
        <button onClick={handleLogout} className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100 transition-colors">
