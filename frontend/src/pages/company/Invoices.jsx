...
import { useGetCompanyInvoicesQuery } from '../../store/api/companyApi'
...
export default function CompanyInvoices() {
  ...
  const { data, isLoading } = useGetCompanyInvoicesQuery({ page, limit: 20 })
  ...
  const handleDownload = async (inv) => {
    ...
    try {
      await downloadInvoicePDF('company', inv.order_id, `${inv.invoice_number}.pdf`)
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-500">All invoices generated for your batches</p>
      </div>
      ...
      {invoices.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-xl border-2 border-dashed border-slate-200 py-16 text-center">
          ...
          <p className="mt-1 text-sm text-slate-400">Invoices appear here after participants complete payment</p>
        </div>
      ) : (
        <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          <table className="min-w-full divide-y divide-slate-100">
            <thead className="bg-slate-50">
              <tr>
                {['Invoice #', 'Participant', 'Batch', 'Amount', 'Paid On', 'Downloads', ''].map((h) => (
            ...
            <tbody className="divide-y divide-slate-50">
              {invoices.map((inv) => (
                <tr key={inv.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <p className="text-sm text-slate-700">{inv.order?.batch?.name}</p>
                    <p className="text-xs text-slate-500">{inv.order?.batch?.program?.type}</p>
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <button
                      ...
                      className="flex items-center gap-1.5 rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs font-medium text-slate-700 hover:bg-slate-50 disabled:opacity-50 transition-colors"
