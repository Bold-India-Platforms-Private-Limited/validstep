      ...
      {invoices.length === 0 ? (
        ...
        <>
          ...
          <div className="space-y-2.5 md:hidden">
            {invoices.map((inv) => (
              <div key={inv.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm active:bg-slate-50">
                <div className="mb-2 flex items-start justify-between gap-2">
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-medium text-slate-900">{inv.order?.batch?.program?.name}</p>
                    <p className="truncate text-xs text-slate-500">{inv.order?.batch?.name} · {inv.order?.batch?.company?.name}</p>
                  </div>
                  <p className="flex-shrink-0 text-sm font-semibold text-slate-900">{formatCurrency(inv.amount)}</p>
                </div>
                <p className="mb-3 rounded bg-slate-50 px-2 py-1 font-mono text-xs text-slate-500">{inv.invoice_number}</p>
            ...
            {pagination.pages > 1 && (
              <div className="flex items-center justify-between px-1 pt-2">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="rounded-full border border-slate-200 px-3.5 py-1.5 text-xs disabled:opacity-40 active:bg-slate-50">Prev</button>
                  <button onClick={() => setPage(p => Math.min(pagination.pages, p + 1))} disabled={page === pagination.pages} className="rounded-full border border-slate-200 px-3.5 py-1.5 text-xs disabled:opacity-40 active:bg-slate-50">Next</button>
                </div>
              </div>
            )}
          </div>
