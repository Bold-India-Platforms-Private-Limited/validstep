...
import {
  ...
  ChevronLeft, ChevronRight, Filter, FileDown, Building2,
} from 'lucide-react'
import {
...
export default function AdminBatchDetail() {
  ...
  const { data: batch, isLoading } = useGetAdminBatchQuery(id)
  const { data: batchStats } = useGetAdminBatchStatsQuery(id)
  const { data: ordersData, isLoading: ordersLoading } = useGetAdminBatchOrdersQuery(
    { batchId: id, status: statusFilter || undefined, page, limit: PAGE_SIZE },
    { skip: activeTab !== 'orders' },
  )
  const { data: certificates, isLoading: certsLoading } = useGetAdminBatchCertificatesQuery(
    id,
    { skip: activeTab !== 'certificates' },
  )
  const [issueCertificates, { isLoading: issuing }] = useIssueAdminCertificatesMutation()
  const [updateTemplate, { isLoading: savingTemplate }] = useSaveAdminBatchTemplateMutation()
  ...
  const selectAllPaid = () => {
    if (selectedOrderIds.length === paidOrders.length) setSelectedOrderIds([])
    else setSelectedOrderIds(paidOrders.map((o) => o.id))
  }
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      const res = await axiosClient.get(`/admin/batches/${id}/orders/export?${params}`)
      ...
      ws['!cols'] = [
        ...
        { wch: 12 }, { wch: 22 }, { wch: 24 }, { wch: 14 }, { wch: 22 }, { wch: 22 },
      ]
      const wb = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(wb, ws, `${batch.name} Orders`.slice(0, 31))
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="flex items-center gap-2 text-sm text-slate-500">
          <Link to="/admin/companies" className="hover:text-slate-700">Companies</Link>
          <span>/</span>
          <Link to={`/admin/companies/${batch.company?.id}`} className="hover:text-slate-700">
            {batch.company?.name}
          </Link>
          <span>/</span>
          <span className="text-slate-700 font-medium">{batch.name}</span>
        </div>
      ...
      <Tabs
        tabs={TABS}
        activeTab={activeTab}
        onChange={(t) => { setActiveTab(t); setPage(1); setSelectedOrderIds([]) }}
      />
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
            {[
              {
                label: 'Total Orders',
                value: batchStats?.orders?.TOTAL ?? batch._count?.orders ?? 0,
                color: 'slate',
                bg: 'bg-slate-50',
              ...
              {
                label: 'Paid Orders',
                value: batchStats?.orders?.PAID ?? 0,
                color: 'emerald',
                bg: 'bg-emerald-50',
              ...
              {
                label: 'Pending',
                value: batchStats?.orders?.PENDING ?? 0,
                color: 'amber',
                bg: 'bg-amber-50',
              ...
              {
                label: 'Failed',
                value: batchStats?.orders?.FAILED ?? 0,
                color: 'red',
                bg: 'bg-red-50',
              ...
              {
                label: 'Certs Issued',
                value: batchStats?.orders?.PAID !== undefined
                  ? (batch._count?.certificates ?? 0)
                  : (batch._count?.certificates ?? 0),
                color: 'primary',
                bg: 'bg-primary-50',
                border: 'border-primary-200',
                text: 'text-primary-800',
                sub: 'text-primary-500',
              },
          ...
          {/* Revenue card (only if there are paid orders) */}
          {(batchStats?.paid_revenue > 0) && (
            <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-5 py-4 flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-emerald-700">Total Revenue Collected</p>
                <p className="text-xs text-emerald-500 mt-0.5">
                  from {batchStats?.orders?.PAID || 0} paid order{batchStats?.orders?.PAID !== 1 ? 's' : ''} · ₹{batch.certificate_price} per certificate
                </p>
              </div>
              <p className="text-3xl font-bold text-emerald-800">
                ₹{(batchStats.paid_revenue).toLocaleString('en-IN')}
              </p>
            </div>
          )}
          ...
          <Card>
            ...
            <CardContent>
              <dl className="space-y-4">
                {[
                  { label: 'Company', value: batch.company?.name },
          ...
          <Card>
            ...
            <CardContent>
              <p className="mb-3 text-sm text-slate-500">
                Public link for participants
              </p>
              <div className="flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2.5">
                <span className="flex-1 truncate text-sm font-mono text-slate-700">{enrollLink}</span>
                <button
                  onClick={copyLink}
                  className="flex-shrink-0 rounded-md p-1.5 text-slate-400 hover:bg-white hover:text-slate-600 transition-colors"
                >
            ...
            <CardContent>
              <div className="rounded-xl border-4 p-8 text-center" style={{ borderColor: primaryColor }}>
                ...
                <p className="text-xs uppercase tracking-widest text-slate-400 mb-2">
                  Certificate of {batch.program?.type === 'INTERNSHIP' ? 'Internship / Fellowship' : 'Completion'}
                </p>
                ...
                certList.map((cert) => (
                  <TableRow key={cert.id}>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <a
                          href={`${window.location.origin}/verify/${cert.verification_hash}`}
                          target="_blank"
                          rel="noopener noreferrer"
